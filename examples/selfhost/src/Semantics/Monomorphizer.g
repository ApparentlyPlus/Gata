...
class Monomorphizer {
    ...
    TopLevel func Instantiate(Template tmpl, List[String] args, String mangled, StringMap[String] binds) {
        ...
        while (i < tmpl.params.Length()) {
            ...
            cMap.Put(prm, self.CTypeOf(spec));
            i = i + 1;
        }
    ...
    /*
     * CTypeOf - The C-type spelling for a Gata type argument, used when substituting type
     * parameters inside native struct fields and native bodies
     */
    public String func CTypeOf(TypeSpec t) {
        match (t) {
            case PtrSpec(p) { return self.CTypeOf(p.inner) + "*"; }
            case NamedSpec(n) {
                let String name = n.Mangled();
                if (name == "void") { return "void"; }
                if (PrimTypes.IsPrim(name)) { return PrimTypes.ToC(name); }
                if (name == BuiltinTypes.Str()) { return self.mangler.Class(BuiltinTypes.Str()) + "*"; }
                if (name == BuiltinTypes.Process() || name == BuiltinTypes.Thread()) { return "void*"; }
                return self.mangler.Class(name) + "*";
            }
