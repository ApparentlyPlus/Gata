...
module Types {
    ...
    public bool func IsNumeric(IrType t) {
        match (t) {
            case IrPrimType(x) { return PrimTypes.IsIntCanon(x.cName) || PrimTypes.IsFloat(x.cName); }
 ...
 * realm's visibility.
 *
 * Realm itself is NOT declared here - see the file header.
 */
 ...
 *
 * TODO(SymbolTable.g): C#'s IrModule also carries the SymbolTable the resolver built. That field
 * is omitted until Semantics/SymbolTable.g is ported; add it there rather than inventing a
...
class IrModule {
    ...
    func _init(List[IrNativeBlock] nativeBlocks, List[IrNativeType] nativeTypes, List[IrClass] classes,
               ...
               List[IrEnum] enums, List[IrType] funcPtrTypes, List[IrUnion] unions) {
