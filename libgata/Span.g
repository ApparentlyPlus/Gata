 ...
 * Build one from String.AsSpan/SubSpan, List[T].AsSpan/SubSpan, or FromRaw over your own buffer.
 * A Span is only as long-lived as what it borrows from - nothing checks that for you, the same
 * deal every raw pointer in Gata already makes.
 */
...
Span[T] func Slice[T](Span[T] s, int start, int len) {
    match (s) {
        case View(ptr, n) {
            ...
            unsafe {
                let base = ptr as char*;
                let shifted = (base + (start as usize) * sizeof(T)) as T*;
                return Span[T].View(shifted, len);
            }
        }
    }
}
